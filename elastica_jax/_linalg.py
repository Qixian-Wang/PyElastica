...
def _batch_matvec(matrix_collection, vector_collection):
    ...
    Notes
    -----
    Jax is really slow here
    """
    blocksize = vector_collection.shape[1]
    output_vector = jnp.zeros((3, blocksize))

    output_vector = jnp.einsum('ijk,jk->ik', matrix_collection[:3, :3, :blocksize], vector_collection[:3, :blocksize])
...
def _batch_matmul(first_matrix_collection, second_matrix_collection):
    ...
    output_matrix = jnp.zeros((3, 3, blocksize))

    output_matrix = output_matrix.at[:3, :3, :blocksize].set(
        jnp.einsum("ijk,jmk->imk",
            first_matrix_collection[:3, :3, :blocksize], second_matrix_collection[:3, :3, :blocksize]
        )
    )
...
def _batch_cross(first_vector_collection, second_vector_collection):
    ...
    output_vector = jnp.empty((3, blocksize))

    output_vector = output_vector.at[0, :blocksize].set(
        first_vector_collection[1, :blocksize] * second_vector_collection[2, :blocksize]
        - first_vector_collection[2, :blocksize] * second_vector_collection[1, :blocksize]
    )


    output_vector = output_vector.at[1, :blocksize].set(
        first_vector_collection[2, :blocksize] * second_vector_collection[0, :blocksize]
        - first_vector_collection[0, :blocksize] * second_vector_collection[2, :blocksize]
    )

    output_vector = output_vector.at[2, :blocksize].set(
        first_vector_collection[0, :blocksize] * second_vector_collection[1, :blocksize]
        - first_vector_collection[1, :blocksize] * second_vector_collection[0, :blocksize]
    )
...
def _batch_dot(first_vector, second_vector):
    ...
    output_vector = jnp.zeros((blocksize))

    output_vector = output_vector.at[:blocksize].set(
        jnp.sum(first_vector[:3, :blocksize] * second_vector[:3, :blocksize], axis=0)
    )

    # blocksize = first_vector.shape[1]
    # output_vector = np.zeros((blocksize))
    #
    # output_vector[:blocksize] = jnp.sum(first_vector[:3, :blocksize] * second_vector[:3, :blocksize], axis=0)
...
def _batch_norm(vector):
    ...
    squared_norms = jnp.sum(vector[:, :blocksize] ** 2, axis=0)
    output_vector = jnp.sqrt(squared_norms)
