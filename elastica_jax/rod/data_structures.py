...
def overload_operator_kinematic_numba(
    ...
    """
    n_nodes = 5
    position_collection = position_collection.at[:3, :n_nodes].set(
        position_collection[:3, :n_nodes] + prefac * velocity_collection[:3, :n_nodes]
    )
...
def overload_operator_dynamic_numba(rate_collection, scaled_second_deriv_array):
    ...
    update_rate_collection = rate_collection[:2, :blocksize] + scaled_second_deriv_array[:2, :blocksize]

    return update_rate_collection
