...
class GravityForces(NoForces):
    ...
    def apply_forces(self, system: SystemType, time=0.0):
        system.external_forces[:] = self.compute_gravity_forces(
    ...
    def compute_gravity_forces(acc_gravity, mass, external_forces):
        ...
        """
        external_forces = inplace_addition(external_forces, _batch_product_i_k_to_ik(acc_gravity, mass))
...
class EndpointForces(NoForces):
    ...
    def apply_forces(self, system: SystemType, time=0.0):
        system.external_forces[:] = self.compute_end_point_forces(
    ...
    def compute_end_point_forces(
        ...
        """
        factor = jnp.minimum(1.0, time / ramp_up_time)
        external_forces = external_forces.at[..., 0].add(
            start_force * factor
        )
        external_forces = external_forces.at[..., -1].add(
            end_force * factor
        )
...
class MuscleTorques(NoForces):
    ...
    def compute_muscle_torques(
        ...
        external_torques[..., 1:] = inplace_addition(
            external_torques[..., 1:],
            _batch_matvec(director_collection, torque)[..., 1:],
        )
        external_torques[..., :-1] = inplace_substraction(
            external_torques[..., :-1],
...
def inplace_addition(external_force_or_torque, force_or_torque):
    ...
    external_force_or_torque = external_force_or_torque.at[:3, :blocksize].add(
        force_or_torque[:3, :blocksize]
    )

    return external_force_or_torque
